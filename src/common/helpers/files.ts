...
export async function base64ToPNG(base64Image: string, nameHint: string): Promise<ConvertToPng> {
  try {
    const matches = base64Image.match(/^data:image\/png;base64,(.+)$/);
    if (!matches) {
      throw new Error('Formato de imagen inválido (se espera image/png en base64)');
    }
...
// buffer-> Contenido del archivo (por ejemplo, Buffer de imagen, PDF, etc.)
// directory-> Ruta completa donde se desea guardar el archivo
// fileName-> Nombre del archivo con extensión (por ejemplo, 'documento.pdf')
// returns Ruta completa del archivo guardado (relativa al sistema)
export async function saveFile(buffer: Buffer, directory: string, fileName: string): Promise<SaveFile> {
  try {
...
export async function generateReceive(studentForms: StudentForms) {
  try {
    ...
    const imagePath = path.join(process.cwd(), 'public', 'images', 'fundetec2.png');
    ...
    // Genera pdf desde un html 

    // const html = fs.readFileSync('src/reports/templates/register-student.html', 'utf8');

    // const content = getHtmlContent(html, body);
    ...
    if (typeProgram.id === 1){
      if (studentForms.studentFormsCycles.length > 0){
        // Se crea el formulario manual y con ciclos
        docDefinition = {
          pageMargins: [20, 20, 20, 20],
          defaultStyle: {
            fontSize: 12
          ...
          content: [
            ...
            // Encabezado con logo e info
            {
              columns: [
                ...
                },
                {
                  ...
                  width: 400,
                  stack: [
                    { text: body.companyName, fontSize: 25, color: 'green', bold: true },
            ...
            // Tabla de datos del estudiante
            {
              table: {
                ...
                body: [
                  [
                    { text: body.titleTable, colSpan: 10, alignment: 'center', bold: true, fontSize: 12, color: 'white', fillColor: 'green', margin: [0, 5, 0, 5] },
                    {}, {}, {}, {}, {}, {}, {}, {}, {}
                  ],
                  [
                    {
                      ...
                      table: {
                        ...
                        body: [
                          [
                            ...
                            { text: 'CONTENIDO', bold: true, fillColor: '#eeeeee' }
                          ],
                          // Aquí insertas dinámicamente los ciclos:
                          ...studentForms.studentFormsCycles.map((cycle: any) => [
                            { text: cycle.cycles.cycleNumber },
                            { text: cycle.contents.contentDescription }
                          ])
                        ]
                      }
                    }, {}, {}, {}, {}, {}, {}, {}, {}, {}
    ...
    // Tecnico
    else if (typeProgram.id === 2){
      // Se crea el formulario manual y lo retorna
      docDefinition = {
        pageMargins: [20, 20, 20, 20],
        defaultStyle: {
          fontSize: 12
    ...
    // Convertir el documento PDF a Buffer
    const docBuffer: Buffer = await new Promise((resolve, reject) => {
      const chunks: any[] = [];
      doc.on('data', (chunk) => chunks.push(chunk));
      doc.on('end', () => resolve(Buffer.concat(chunks)));
      doc.on('error', reject);
      doc.end(); // importante para finalizar el stream
    });

    // almacena registro en bodega
    ...
    if (!savedFile.status){
      throw new NotFoundException(`Hubo un error al momento de almacenar imagen de la asignatura`);
    }
...
export async function createPdf(documentDefinition: TDocumentDefinitions, options: BufferOptions = {}) {
  const fonts = {
    Roboto: {
